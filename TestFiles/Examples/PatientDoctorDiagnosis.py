...
events = ["*"]
filters = "and (Patient.DateOfBirth >= 1950 and Patient.DateOfBirth <= 1970) and (Patient.PatientSerNum >= 1 and Patient.PatientSerNum <= 2500) and (Patient.Sex REGEXP \"Male\") and (MONTH(PERIOD.LastUpdated) = 01 or MONTH(PERIOD.LastUpdated) = 02 or MONTH(PERIOD.LastUpdated) >= 03 and MONTH(PERIOD.LastUpdated) <= 10  or MONTH(PERIOD.LastUpdated) = 11) and (YEAR(PERIOD.LastUpdated) >= 2010 and YEAR(PERIOD.LastUpdated) <= 2015 ) "
...
for p, pval in results.get_type("PATIENT").iteritems():
	...
	for d in pval["DOCTOR"]: 
		printActor(results,  d, "DOCTOR", tabCount=2)
		for diag in d["DIAGNOSIS"]: 
			printActor(results, diag, "DIAGNOSIS", tabCount=3)
